-- Keep the decisions trigram index reachable through the cortex_app RLS barrier.
--
-- PostgreSQL cannot push non-leakproof ILIKE below row-security policies. On the
-- Marlow production corpus that made the RLS role seq-scan ~198k decisions even
-- though idx_decisions_summary_trgm existed. This deliberately narrow function
-- runs the fixed query as its postgres owner, but returns rows only when the
-- requested project exactly matches the session scope set by acquire_scoped().
-- It accepts no identifiers or dynamic SQL and exposes only search-result fields.
